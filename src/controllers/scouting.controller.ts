...
export async function getScoutingRankingController(req: Request, res: Response) {
  ...
  const nationalityRaw = typeof req.query.nationality === "string" ? req.query.nationality.trim() : "";
  const nationality = nationalityRaw || undefined;
  ...
  const cacheKey = `scouting:ranking:${position ?? "all"}:${leagueId ?? ""}:${nationality ?? ""}:${ageMin ?? ""}:${ageMax ?? ""}:${overallMin ?? ""}:${label ?? ""}:${limit ?? 50}`;

  const result = await withCache(cacheKey, 120, () =>
    getScoutingRanking({ position, leagueId, nationality, ageMin, ageMax, overallMin, label, limit }),
  );
